...
- `tests/handlers/test_premium_topup_flow.py` — Python-модуль
  Классы: `TestPurchasableSquads` (4 методов), `TestButtonVisibility` (3 методов), `TestTopupEntryPoint` (3 методов), `TestSquadSelection` (1 методов), `TestBuyRefusesWithoutCharging` (3 методов), `TestBuySucceeds` (2 методов), `TestCeilingIsDisplayOnly` (3 методов)
  Функции: нет
