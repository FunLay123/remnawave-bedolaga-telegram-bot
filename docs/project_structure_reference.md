...
- `app/cabinet/schemas/tariffs.py` — Python-модуль
  Классы: `PeriodPrice` (1 методов), `ServerTrafficLimit` (1 методов), `ServerInfo`, `PromoGroupInfo`, `TariffListItem`, `TariffListResponse`, `TariffDetailResponse`, `ExternalSquadInfoResponse`, `TariffCreateRequest`, `TariffUpdateRequest`, `TariffSortOrderRequest`, `TariffToggleResponse`, `TariffTrialResponse`, `TariffStatsResponse`, `SyncSquadsResponse`
  Функции: нет
...
- `app/services/premium_traffic_service.py` — Python-модуль
  Классы: `PremiumTrafficService` (35 методов)
  Функции: нет
