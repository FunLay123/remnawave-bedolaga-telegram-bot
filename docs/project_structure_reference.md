...
- `app/handlers/subscription/traffic.py` — Python-модуль
  Классы: нет
  Функции: `handle_add_traffic`, `handle_reset_traffic`, `confirm_reset_traffic`, `refresh_traffic_config`, `get_traffic_packages_info`, `select_traffic`, `add_traffic`, `handle_premium_traffic_topup` — Точка входа: кнопка «Премиум-трафик» на экране докупки обычного трафика., `handle_premium_traffic_squad` — Пользователь выбрал сервер из списка — показываем его пакеты докупки., `buy_premium_traffic` — Списание за докупленный премиум-трафик. Порядок шагов — как в `add_traffic`., `handle_no_traffic_packages`, `handle_switch_traffic`, `confirm_switch_traffic`, `execute_switch_traffic`
...
- `app/utils/premium_traffic.py` — Python-модуль
  ...
  Функции: `parse_squad_record` — Разобрать одну запись `server_traffic_limits` целиком, не отбрасывая нулевой лимит., `parse_premium_squad` — Разобрать одну запись `server_traffic_limits`., `parse_premium_squads` — Разобрать весь `server_traffic_limits` тарифа., `get_premium_squads_for_tariff` — Премиум-сквады тарифа. Безопасно принимает None вместо тарифа., `get_squad_record_for_tariff` — Полная запись одного сквада вне зависимости от текущего значения лимита., `exclude_limited_squads` — Убрать из набора сквады, снятые за исчерпание премиум-лимита., `effective_panel_squads` — Набор сквадов для отправки в панель с учётом снятых премиум-сквадов.
...
- `tests/cabinet/test_admin_premium_traffic.py` — Python-модуль
  Классы: `TestPremiumReset` (6 методов), `TestPremiumClose` (8 методов), `TestPremiumReopen` (4 методов), `TestRegularReset` (5 методов)
  Функции: нет
...
- `tests/cabinet/test_server_traffic_limit_roundtrip.py` — Python-модуль
  Классы: нет
  Функции: `test_premium_topup_settings_survive_a_tariff_save`, `test_every_stored_key_is_declared_in_the_schema` — Страховка на будущее: новый ключ в JSON без поля в схеме — потеря данных., `test_saved_form_is_still_readable_by_the_domain_parser` — Круг через кабинет не должен ломать разбор на стороне воркера., `test_legacy_record_without_topup_fields_gets_safe_defaults` — Тарифы, заведённые до появления премиум-докупки, не должны падать., `test_get_reports_topup_disabled_when_no_packages` — topup_enabled=true без пакетов — это выключенная докупка, так и отдаём., `test_partial_update_keeps_other_squads` — PATCH одного сквада не должен стирать настройки остальных., `test_write_rejects_malformed_packages` — Мусор в пакетах докупки отклоняем на записи, а не терпим до следующего чтения., `test_servers_and_server_limits_response_agree_on_a_zero_limit_squad` — `servers[]` и `server_traffic_limits` ответа не должны расходиться во мнении об одном скваде.
...
- `tests/handlers/test_admin_tariffs_premium.py` — Python-модуль
  Классы: нет
  Функции: `test_parse_premium_packages_accepts_free_package` — В отличие от обычной докупки, премиум-докупка допускает бесплатный пакет., `test_parse_premium_packages_rejects_negative_price`, `test_parse_premium_packages_ignores_garbage_tokens`, `test_zero_limit_formats_as_no_separate_limit_not_closed_access`, `test_premium_squad_screen_reads_through_shared_parser_not_raw_dict` — Три исторические формы должны читаться одинаково через parse_premium_squads., `test_premium_squad_screen_marks_zero_limit_as_not_premium_and_never_closed`, `test_topup_status_does_not_imply_availability_when_kill_switch_is_off`, `test_saving_limit_for_one_squad_preserves_other_squad_settings` — Партиальное обновление не должно стирать настройки другого сквада., `test_saving_packages_preserves_own_squads_limit_and_name` — Правка одного поля сквада не должна стирать остальные поля того же сквада., `test_editing_other_field_after_zeroing_limit_preserves_stored_name_and_packages` — Обнуление лимита не должно стирать остальные поля при следующей правке., `test_editing_other_field_after_zeroing_limit_preserves_legacy_shape_without_limit_key` — То же самое для исторической формы записи без ключа `traffic_limit_gb`., `test_removing_squad_from_premium_set_zeroes_limit_key_stays` — Снятие премиума со сквада — это лимит=0, а не удаление ключа из карты., `test_garbage_limit_input_is_rejected_without_write`, `test_negative_limit_input_is_rejected_without_write`, `test_all_garbage_packages_input_is_rejected_without_write`, `test_overlong_name_input_is_rejected_without_write`, `test_enabling_topup_without_packages_is_rejected`, `test_toggle_topup_persists_only_topup_flag_when_packages_exist`, `test_tariff_view_keyboard_has_premium_squads_entry_point`, `test_premium_squad_keyboard_uses_short_callback_data_within_telegram_limit`
- `tests/handlers/test_admin_users_premium.py` — Python-модуль
  Классы: `TestCollectPremiumTrafficRows` (5 методов), `TestKillSwitch` (1 методов), `TestGrantPremiumTraffic` (3 методов), `TestCloseAndReopenFromCard` (4 методов)
  Функции: `test_no_limit_is_not_active_and_not_closed`, `test_active_state_without_history`, `test_exhausted_by_usage_is_not_closed`, `test_admin_closed_wins_over_exhaustion`, `test_admin_closed_wins_even_when_usage_alone_would_not_be_exhausted` — closed_at — durable-флаг, проверяется первым независимо от used_bytes., `test_reopened_but_not_yet_restored_is_a_distinct_state_from_closed` — Открыт, но воркер ещё не вернул сквад в панель — НЕ 'закрыто администратором'., `test_row_rendering_never_calls_reopened_pending_state_closed_by_admin`, `test_row_rendering_closed_by_admin_says_so_explicitly`, `test_row_rendering_no_limit_never_says_closed`, `test_extract_premium_squad_context_without_subscription_id`, `test_extract_premium_squad_context_with_subscription_id`, `test_extract_premium_squad_value_context_without_subscription_id`, `test_extract_premium_squad_value_context_with_subscription_id`, `test_callback_data_stays_within_telegram_limit_for_worst_case_ids`
...
- `tests/handlers/test_premium_topup_flow.py` — Python-модуль
  Классы: `TestPurchasableSquads` (4 методов), `TestButtonVisibility` (3 методов), `TestTopupEntryPoint` (3 методов), `TestSquadSelection` (1 методов), `TestBuyRefusesWithoutCharging` (3 методов), `TestBuySucceeds` (2 методов), `TestStateWording` (2 методов), `TestCeilingIsDisplayOnly` (3 методов)
  Функции: нет
...
- `tests/services/test_grace_respects_premium_limits.py` — Python-модуль
  Классы: нет
  Функции: `test_the_panel_target_drops_a_squad_whose_premium_quota_is_exhausted` — Исчерпанный премиум-сквад из цели уходит, остальные — нет., `test_the_panel_target_keeps_everything_the_customer_is_still_entitled_to` — Фильтр вычитающий: пока лимит не исчерпан, набор не меняется., `test_canonical_panel_payload_does_not_regrant_the_exhausted_squad` — Payload собирается из уже отфильтрованной цели, а не из прав., `test_applying_canonical_billing_does_not_regrant_the_exhausted_squad` — Полный путь `apply_billing_state`: и запись, и сверка её результата., `test_applying_canonical_billing_keeps_the_squads_still_paid_for` — Обратная страховка: без исчерпания в панель уходит полный набор прав., `test_recovery_after_payment_does_not_regrant_the_exhausted_squad` — Названное место бага: `apply_recovered_grace_update_locked`., `test_a_premium_grant_during_an_overlay_does_not_conflict_the_session` — Докупка трафика или начисление админом не должны обрывать grace., `test_the_worker_limiting_mid_pass_does_not_conflict_the_session` — Обратное направление — гонка «сессия открылась посреди прохода».
...
- `tests/services/test_premium_squads_are_filtered_everywhere.py` — Python-модуль
  Классы: нет
  Функции: `test_every_panel_squad_write_is_guarded`, `test_the_guard_is_actually_used` — Страховка от обратного: правило есть, а применять его перестали., `test_grace_exception_list_does_not_rot` — Список исключений должен указывать на существующие места отправки., `test_the_write_door_is_guarded` — Исключение для payload держится на том, что фильтр стоит в writer., `test_every_squad_writer_filters_limited_squads`, `test_account_patcher_does_not_touch_squads` — `patch_panel_account` фильтровать нечего — и он не должен знать о сквадах., `test_guard_is_reachable_from_the_writer` — Страховка от обратного: правило есть, а импорт потеряли., `test_every_squad_patch_names_its_subscription` — Каждый вызов `patch_panel_squads` обязан передать `subscription_id`., `test_squad_patch_scan_finds_the_callers` — Сторож выше не должен проходить вхолостую, если сканер перестал видеть вызовы.
...
- `tests/services/test_update_panel_user_grace_safe_signal.py` — Python-модуль
  Классы: нет
  Функции: `test_deferred_write_is_distinguishable_from_applied` — Сквад-only апдейт под открытым оверлеем — запись отложена, а не применена., `test_deferred_result_still_reads_like_the_panel_card` — Безразличный вызывающий не ломается: обёртка прозрачна на чтение и на `if`., `test_applied_write_returns_the_panel_object_unchanged` — Без открытого оверлея возврат — объект панели как был, без обёртки., `test_partially_applied_write_still_counts_as_deferred` — Незащищённая часть уезжает в панель, защищённая — откладывается., `test_wrapper_does_not_turn_an_empty_panel_answer_into_a_success` — Пустой ответ панели остаётся falsy и под обёрткой., `test_update_without_protected_fields_is_applied_not_deferred` — Оверлей открыт, но защищённых полей в запросе нет — откладывать нечего., `test_real_indifferent_caller_still_records_panel_identity` — Настоящий безразличный вызывающий на отложенном возврате работает как прежде., `test_anything_unmarked_reads_as_applied` — Непомеченный объект — применённая запись, а не отложенная.
