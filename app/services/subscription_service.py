        ...
        try:
            ...
            async with self.get_api_client() as api:
                ...
                panel_payload = replace(
                    ...
                    hwid_device_limit=hwid_limit,
                    external_squad_uuid=ext_squad_uuid,
                    # Готовый payload уходит не только в push_subscription, но и
                    # в грейс-ветку ниже — она пишет в панель сама, минуя
                    # writer. Поэтому снятые за перерасход премиум-сквады
                    # убираем здесь, а не полагаемся на фильтр писателя.
                    active_internal_squads=tuple(
                        await effective_panel_squads(subscription.id, subscription.connected_squads, db=db) or ()
                    ),
                )
