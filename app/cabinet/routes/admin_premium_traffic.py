...
закрытие — ``/reopen``: она возвращает ``used_bytes`` к нулю и очищает
``closed_at``, а не подделывает расход — следующий проход воркера пересчитает
его от статистики панели заново. ``is_limited`` при этом снимает воркер, а не
``/reopen``: флаг — это то, по чему обе ветки возврата (``_restore_squad`` у
премиального сквада и ``_clear_orphan`` у осиротевшего) узнают, что сквад надо
вернуть в панель, и снимается он там же, где сквад реально уезжает в панель.
"""
...
async def reopen_premium_access(
    ...
    Ответ отдаёт не ``is_limited``, а два разных факта: закрытие снято прямо
    сейчас (``closed``), а возврат сквада в панель поставлен в очередь к воркеру
    (``pending_panel_restore``). До фикса здесь стоял голый ``is_limited``, и он
    вводил в заблуждение: оператор видел «не ограничен» в тот момент, когда
    панель сквад ещё не отдавала.
    """
    subscription = await _load_subscription(db, subscription_id)
    ...
    return {
        ...
        'squad_uuid': request.squad_uuid,
        'closed': False,
        'pending_panel_restore': bool(state.is_limited),
    }
...
async def _reopen_access(
    ...
    """Снять закрытие: обнулить расход и поручить воркеру вернуть сквад в панель.
    ...
    **``is_limited`` намеренно не снимаем.** Панель отсюда не дёргаем (как и
    `/close`, и `grant`: единственный писатель ``activeInternalSquads`` —
    воркер, с его grace-оверлеем и единым путём отказа), а вернуть сквад в
    панель можно только по поднятому флагу — обе ветки возврата читают именно
    его: `_restore_squad` у сквада, который в тарифе ещё премиальный
    (``is_limited and not is_exhausted``, а расход мы только что обнулили), и
    `_clear_orphan` у осиротевшего (``needs_restore = orphan.is_limited``).
    Снимая флаг здесь, реопен стирал бы единственный признак, по которому сквад
    полагается вернуть: для премиального сквада возврат ещё вытягивала сверка с
    панелью, а для осиротевшего ближайший проход просто удалял строку, ничего в
    панель не отправив, — и доступ не возвращался вообще никогда. Флаг снимет
    та же ветка, что и отправит сквад в панель, внутри своей точки сохранения:
    база не утверждает возврата раньше, чем его подтвердила панель.

    Исключение — сквад, которого нет в ``connected_squads``: подписке он и так
    не положен, в панель не уезжает и возвращать его нечем, а целью воркера
    (`_collect_targets` проверяет ``connected_squads``) такая строка не станет и
    осиротевшей не считается, пока лимит в тарифе есть. Флаг остался бы поднятым
    навсегда, поэтому здесь снимаем его сами — ровно по тому же условию, по
    которому `_clear_orphan` решает, что возвращать нечего.

    ``closed_at`` снимаем в любом случае: пока он стоит, `start_new_period` и
    `add_extra_bytes` не тронут ``is_limited``, а `_clear_orphan` не тронет
    строку — то есть ни одна ветка возврата не сработала бы.
    """
    ...
    if squad_uuid not in (getattr(subscription, 'connected_squads', None) or []):
