...
#
# Админское закрытие сквада (`closed_at`) с Task 13 проверяет и сам сервис —
# `quote_premium_topup` отказывает с `PremiumTopupError('squad_closed', ...)`,
# `apply_premium_topup` перепроверяет то же условие под блокировкой строки,
# вместе с потолком докупки (это единственная точка входа для покупки, что в
# боте, что в кабинете). Проверки этого экрана ниже теперь избыточны, но не
# лишние: `_get_purchasable_premium_squads` убирает закрытые сквады из списка
# ДО того, как сервис вообще увидит запрос (не показывать кнопку — не то же
# самое, что отклонить нажатие), а `buy_premium_traffic` перепроверяет это же
# условие прямо перед списанием ради читаемого текста отказа пользователю —
# без неё та же ошибка дошла бы до него через общий `except PremiumTopupError`
# с тем же смыслом, но менее заметно на этом экране.
...
async def _get_purchasable_premium_squads(db: AsyncSession, subscription) -> list[dict]:
    ...
    подписке). Здесь она дополнена условием закрытия администратором
    (`closed_at`): сервис (`quote_premium_topup`) его тоже теперь отклоняет, но
    отфильтровать заранее правильнее, чем дать пользователю дойти до экрана
    выбора пакета ради гарантированного отказа на последнем шаге.
    """
...
async def buy_premium_traffic(callback: types.CallbackQuery, db_user: User, db: AsyncSession, state: FSMContext = None):
    ...
    # нажатием кнопки администратор мог закрыть доступ. С Task 13 это отклонил
    # бы и сам `quote_premium_topup` (`PremiumTopupError('squad_closed', ...)`,
    # см. `except PremiumTopupError` ниже) — эта проверка избыточна, но
    # оставлена ради текста отказа, специфичного для этого экрана, а не общего
    # `error.message` через `except`.
